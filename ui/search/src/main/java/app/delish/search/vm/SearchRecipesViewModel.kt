...
internal class SearchRecipesViewModel @Inject constructor(
    ...
    private val _apiSearchResults = MutableStateFlow<List<RecipesItem>>(emptyList())
    val apiSearchResults: StateFlow<List<RecipesItem>> = _apiSearchResults
    ...
    fun fetchRecipesFromApi(query: String) {
        viewModelScope.launch {
            try {
                val response = recipesRepository.fetchRecipesFromApi(query)
                _apiSearchResults.value = response.results.map { recipe ->
                    RecipesItem(
                        id = recipe.id,
                        title = recipe.title,
                        image = recipe.image
